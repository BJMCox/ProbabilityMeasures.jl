...
`Geometric`, `MvNormal`, and `Multinomial`.
...
`Laplace(μ, b)`, `Categorical(p)`, `Cauchy`, `Bernoulli(p)`, `Binomial(n, p)`, and
`Geometric(p)` each support:
...
`Uniform`, `Cauchy`, `Laplace`, `Categorical`, `Bernoulli`, `Binomial`, `Geometric`,
`MvNormal`, and `Multinomial`. Transformed or composite measures and Distributions.jl
