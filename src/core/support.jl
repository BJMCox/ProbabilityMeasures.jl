...
so it cannot pin a precision or leak a `Float64` into an otherwise `Float32`
computation, and it costs nothing to pass into a kernel.
"""
...
outside the support on its own, so densities do not need to call this. It exists for
samplers, transforms, and validation.
"""
